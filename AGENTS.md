...
- **Axios clients must include emoji-coded debug interceptors with sanitized headers.** Every axios client instance must have both request and response interceptors with emoji-coded console logging: 🔵 for outgoing requests (method, URL, data — never headers), 🟢 for successful responses (status, data), 🔴 for errors (message, status, URL). Never create an axios client without these debug interceptors. Headers must be excluded from request logs to prevent leaking API keys, auth tokens, or other sensitive credentials.

  **Good:**

  const client = axios.create({ baseURL: Env.API\_URL });
  ...
  data: config.data,
  // headers intentionally omitted — never log sensitive credentials
  });
  return config;
  },
  (error) => {
  ...
  **Bad:**
  ...
  url: (config.baseURL || '') + (config.url || ''),
  data: config.data,
  headers: config.headers,  // ❌ leaks X-API-Key, Authorization tokens
  });
  return config;
  });

  // Missing response interceptors — incomplete debug coverage
